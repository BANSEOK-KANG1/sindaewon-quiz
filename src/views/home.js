...
export async function renderHome(root) {
  ...
  const must = questions.filter((q) => q.tags?.includes("필수")).length;
  ...
  const chong = counts.chongshin || {};
  const chongTotal = (chong["성경"] || 0) + (chong["영어"] || 0);
  ...
  root.innerHTML = `
    ...
    <section class="strategy-card card">
      <p class="mode-kicker">오늘 전략 (가중치 우선)</p>
      ...
      <div class="strategy-actions" style="margin-top:0.75rem">
        <a class="btn btn-primary" href="#/quiz?mode=study&tags=${encodeURIComponent(typeFocus.tag)}&subject=성경&count=15&auto=1">유형 15</a>
        <a class="btn btn-secondary" href="#/quiz?mode=study&tags=기출,${encodeURIComponent(bibleFocus.tag)}&subject=성경&count=10&auto=1">권별 10</a>
    ...
    <section class="mode-grid home-modes">
      <a class="mode-card mode-study" href="#/quiz?mode=study&tags=필수&subject=성경&count=15&auto=1">
        <span class="mode-kicker">필수</span>
        <strong>필수 가중 15</strong>
        <span>신_유형·다빈도</span>
      </a>
      <a class="mode-card" href="#/quiz?mode=exam&tags=필수&count=25&auto=1">
      ...
      <a class="mode-card" href="${wrong ? "#/wrong-note" : "#/quiz?mode=exam&tags=기출&subject=성경&count=20&auto=1"}">
        <span class="mode-kicker">${wrong ? "오답" : "기출"}</span>
        <strong>${wrong ? `틀린 ${wrong}건` : "기출 시험 20"}</strong>
        <span>${wrong ? "다시 풀기" : "통합 기출"}</span>
      </a>
    </section>

    <section class="card seminary-card" style="margin-top:1rem">
      <h2>${SEMINARIES.chongshin.name}</h2>
      <p class="card-meta">성경 ${chong["성경"] || 0} · 영어 ${chong["영어"] || 0}</p>
      <p class="stat">총 <strong>${chongTotal.toLocaleString("ko-KR")}</strong> / 전체 ${total.toLocaleString("ko-KR")} · 필수 ${must.toLocaleString("ko-KR")}</p>
      <p class="muted small">장신 기출은 아직 없습니다. 수집되면 같은 방식으로 추가합니다.</p>
      <div class="card-actions">
        <a class="btn btn-primary" href="#/quiz?seminary=chongshin&restart=1">학습 시작</a>
        <a class="btn btn-secondary" href="#/browse?seminary=chongshin">탐색</a>
      </div>
    </section>

    <p class="muted small home-footnote">기출 ${grad.toLocaleString("ko-KR")} · 필수 ${must.toLocaleString("ko-KR")} · 단어장 ${vocab.toLocaleString("ko-KR")}. 학습 탭에서 <strong>유형 가중치 → 권별 → 실전</strong> 순으로 공략하세요.</p>
  `;
